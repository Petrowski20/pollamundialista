...
function buildWhatsAppUrl(matchLabel: string, missingNames: string[]): string {
  const appUrl = window.location.origin
  const names = missingNames.join(', ')
  const text =
    `⚽ ¡Poned la porra, vagos!\n\n` +
    `El partido *${matchLabel}* empieza pronto y aún no habéis votado:\n` +
    `👤 ${names}\n\n` +
...
export default function RemindersView({ matches }: { matches: MatchWithVoters[] }) {
  const totalMissing = matches.reduce((acc, m) => acc + m.faltanPorVotar.length, 0)
    ...
    <div className="w-full space-y-6">
      ...
      <div className="flex items-start justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
            Próximos {matches.length} partido{matches.length !== 1 ? 's' : ''} pendientes y su estado de predicciones.
          </p>
        </div>

        <div className="flex items-center gap-3">
          <div className="bg-red-50 dark:bg-red-900/20 border border-red-100 dark:border-red-800/40 rounded-xl px-4 py-2 text-center min-w-[72px]">
            <p className="text-xl font-bold text-red-700 dark:text-red-400">{totalMissing}</p>
            <p className="text-xs text-red-600 dark:text-red-500">Faltan votos</p>
          </div>
        </div>
      </div>
      ...
      {matches.length === 0 && (
        <div className="bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-800 p-14 text-center">
          ...
          <p className="font-semibold text-gray-700 dark:text-gray-300">No hay partidos pendientes próximos</p>
          <p className="text-sm text-gray-400 dark:text-slate-500 mt-1">Añade partidos desde la pestaña de Partidos.</p>
        </div>
      )}
      ...
      {matches.map((match) => {
        ...
        const allVoted = match.faltanPorVotar.length === 0
        ...
        return (
          ...
          >
            ...
            <div className="bg-gray-50 dark:bg-slate-800/60 px-5 py-4 border-b border-gray-100 dark:border-slate-800 flex flex-wrap items-center justify-between gap-3">
              <div className="flex items-center gap-3 min-w-0">
                {match.home_team?.flag_emoji && getFlagUrl(match.home_team.flag_emoji)
                  ? <img src={getFlagUrl(match.home_team.flag_emoji)} alt={homeLabel} className="w-8 h-5 object-cover rounded-sm shadow-sm flex-shrink-0" />
                  : null}
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-gray-400 dark:text-gray-500">
                    {stageLabel}{groupSuffix} &middot; {dateStr} a las {timeStr}
                  </p>
                </div>
                {match.away_team?.flag_emoji && getFlagUrl(match.away_team.flag_emoji)
                  ? <img src={getFlagUrl(match.away_team.flag_emoji)} alt={awayLabel} className="w-8 h-5 object-cover rounded-sm shadow-sm flex-shrink-0" />
                  : null}
              </div>
              ...
              {!allVoted && (
                <button
                  onClick={() =>
                    window.open(buildWhatsAppUrl(matchLabel, missingNames), '_blank', 'noopener,noreferrer')
                  }
                  className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold bg-[#25D366] hover:bg-[#1ebe5d] text-white transition-all active:scale-95 shadow-sm whitespace-nowrap flex-shrink-0"
                >
                  ...
                  Avisar a despistados
                </button>
              )}

              {allVoted && (
                <span className="flex items-center gap-1.5 text-xs font-semibold text-emerald-700 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-800/40 px-3 py-1.5 rounded-full">
                  ✓ Todos han votado
                </span>
              )}
            </div>
            ...
            {/* Cuerpo: dos columnas */}
            <div className="grid sm:grid-cols-2 divide-y sm:divide-y-0 sm:divide-x divide-gray-100 dark:divide-slate-800">

              {/* Han votado */}
              <div className="p-5">
                <div className="flex items-center gap-2 mb-3">
                  <span>✅</span>
                  <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300">Han votado</h3>
              ...
              <div className="p-5">
                <div className="flex items-center gap-2 mb-3">
                  <span>❌</span>
                  <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300">Faltan por votar</h3>
